...
points and biases team control share by 12.5--13.4 points across the two
...
error roughly in half (to 12.2--13.3 points) and share bias to below 5 points,
consistently across both matches and viewport widths from 36\,m to 60\,m. In
...
learned method should beat, and (iii) the first scoring, to our knowledge, of
...
(games 1/2), matching our real broadcast clips (10--16 visible) and the
12.8$\pm$3.7 reported by~\citet{omidshafiei2022}. Sensitivity is reported for
$W \in \{36, 44, 52, 60\}$\,m.
...
toward the visible-team mean with exponential weight $e^{-\Delta t/\tau}$,
$\tau = 8$\,s.
...
is subtracted before averaging, the systematic subset bias cancels rather than
accumulates. When fewer than three voters are available, B4 falls back to the
formation anchor (B2), and B2 in turn to last-seen (B1) for players without a
stored offset; such fallback frames are included in the reported numbers.
\end{description}
...
bias 4.5--4.7\pp{}, roughly a third of B0. Figure~\ref{fig:panels} visualises
...
consistent in both matches; B4's absolute share-bias gain is largest in the
...
in both matches; B4 reduces share bias to roughly a third of B0 (28--39\%
across widths), with the largest absolute share-bias gains in the narrowest
...
share of hidden observations & \multicolumn{2}{c}{43--47\%} & 53--57\% \\
\bottomrule
\end{tabular}
\end{center}

Within the $\le$9.6\,s regime that the Graph Imputer's protocol scores, a
training-free online method reaches 3.3--8.9\,m median error --- of the same
order as learned bidirectional models operating with strictly more information,
which remain more accurate within their own regime.
The majority of hidden-player observations in our benchmark (53--57\%),
however, lies \emph{beyond} 9.6\,s: long occlusions of far-side defenders
...
vs.\ the official 26.6 baseline~\citep{somers2024soccernet} (challenge-winning
systems with GPU appearance re-identification reach substantially higher; our
pipeline is CPU-only). Off-screen players
...
Section~\ref{sec:benchmark}). Positive SCI: the possession creates space even
without shots; zero or negative: dead possession. On two event-flagged junk-possession
...
verdicts can move by 15.6--17.2 SCI points under imputation --- larger than the gap
...
ladder that we leave to future work. Finally,
