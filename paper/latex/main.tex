...
practice, inflates hidden-zone pitch-control error to 25.6--26.9 percentage
points and shifts the reported team control share by 12.5--13.4 points on
average across the two matches. We then evaluate a ladder of
...
roughly halves hidden-zone error (to 12.2--13.3 points) and cuts the
control-share error to about a third of the ignore policy at every viewport
width from 36\,m to 60\,m in both matches (4.5--4.7 points at $W=44$\,m). In
...
order as learned models that additionally exploit future observations; 53--57\%
...
error and cuts control-share error to a third of the ignore-policy baseline,
without offline training, future observations, or GPU
...
which turns out to hold the majority (53--57\%) of hidden-player observations
in our benchmark.
...
pitch height. At $W = 44$\,m the viewport shows 14.6--14.9 players on average
(games 1/2), within the range of our real broadcast clips (10--16 visible) and
...
among methods on them; the map MAE (2) is reported for completeness.
...
removes). When hidden, place the player at the current visible centroid plus the
stored offset.
...
Kept largely as a negative result (Section~\ref{sec:results}).
...
$W = 44$\,m the voting path produces 70--73\% of B4's hidden-player estimates
(B2 fallback 27--30\%, B1 0\%), so the reported performance is a mix in those
proportions.
\end{description}
...
matches). Lower is better throughout; bold marks the best method per column.
B0 has no position error since it places no players.}
\label{tab:ladder}
\small
...
& \shortstack{hidden ctrl\\MAE (pp)} & \shortstack{share\\err.\ (pp)} & \shortstack{median pos.\\err.\ (m)}
& \shortstack{hidden ctrl\\MAE (pp)} & \shortstack{share\\err.\ (pp)} & \shortstack{median pos.\\err.\ (m)} \\
\midrule
B0 ignore        & 26.9 & 13.4 & ---  & 25.6 & 12.5 & ---  \\
B1 last-seen     & 22.1 & 10.6 & 19.6 & 20.0 &  9.5 & 17.9 \\
B2 anchor        & 15.7 &  6.2 & 13.6 & 14.6 &  5.4 & 12.8 \\
B3 +velocity     & \textbf{12.8} &  5.7 & 14.6 & \textbf{11.8} &  4.6 & 14.0 \\
B4 centroid vote & 13.3 & \textbf{4.7} & \textbf{11.6} & 12.2 & \textbf{4.5} & \textbf{10.0} \\
\bottomrule
\end{tabular}
\end{table}
...
25.6--26.9\pp{} and shifts the headline control share by 12.5--13.4\pp{} on
average.
The naive last-seen policy removes only about a fifth of it. The formation anchor
...
on both decision-relevant metrics: position error 10.0--11.6\,m median and
share error 4.5--4.7\pp{}, roughly a third of B0. Full-pitch control MAE
follows the same ordering (B0 17.2--18.6\pp{} $\to$ B4 9.2--10.5\pp{}).
Figure~\ref{fig:panels} visualises
the effect on a single frame: the hidden zone misassigned by B0 is largely
restored by B4.
...
pushing them away. The latest formation snapshot is the better prior. B3 does
retain a small edge on hidden-zone map MAE (its exponentially averaged anchor
smooths the control field), but it is worse where decisions are made ---
position error and control share --- so we adopt B4.
...
control-share error, both games). Right: viewport-width sensitivity --- B4
(solid) vs.\ B0 (dashed) across $W = 36$--$60$\,m. The ordering is monotone and
consistent in both matches; B4's absolute share-error gain is largest in the
narrowest (hardest) viewports.}
\label{fig:ladder}
\end{figure}
...
and control-share error, B0 vs.\ B4.}
\label{tab:sens}
\small
...
in both matches; B4 reduces the share error to roughly a third of B0 (28--39\%
across widths), with the largest absolute share-error gains in the narrowest
(hardest) viewports --- the method is most valuable exactly when observation is
poorest.
...
Visible players & 12.8 $\pm$ 3.7 of 22 & 14.6--14.9 of 22 ($W=44$\,m) \\
...
\midrule
B4 median position error & 3.3--3.7\,m & 7.2--8.9\,m & 15.6--15.9\,m \\
share of hidden samples (frame-weighted) & \multicolumn{2}{c}{43--47\%} & 53--57\% \\
\bottomrule
\end{tabular}
\end{center}
...
The majority of hidden (frame, player) samples in our benchmark (53--57\%,
frame-weighted --- long episodes dominate this count), however, lies
...
scored. It is also where our absolute errors (15.6--15.9\,m median) show the
...
large and consistent across both matches and all viewport widths. The simulated
viewport pans but does not zoom or tilt; zoom changes the number of visible
players, so real-broadcast occlusion statistics (including the 53--57\%
long-occlusion share) may differ from the simulated ones. A fixed
formation-template prior (mean role positions) is a further natural rung of the
ladder that we leave to future work. All hyperparameters ($\tau = 8$\,s, EMA
weight 0.1, the three-voter threshold, the 1.5\,s blend constant) are round
values fixed once during development and not optimised by search --- but they
were chosen while working with these same two matches, so evaluation on
independent held-out matches remains future work. Finally,
